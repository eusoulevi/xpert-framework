...
public abstract class BaseDAOImplOld<T> implements BaseDAO<T> {
    ...
    public BaseDAOImplOld() {
        try {
            Object object = ((ParameterizedType) getClass().getGenericSuperclass()).getActualTypeArguments()[0];
    ...
    public List<T> listAll(String order) {
        Query query = new QueryBuilderOld(entityClass, entityManager, order).createQuery();
        return query.getResultList();
    }

    @Override
    public List<T> listAll(Class clazz, String order) {
        Query query = new QueryBuilderOld(clazz, entityManager, order).createQuery();
        return query.getResultList();
    }
    ...
    public Object findAttribute(String attributeName, Long id) {
        ...
        QueryBuilderOld queryBuilder = new QueryBuilderOld(entityClass, entityManager);
        queryBuilder.add(new Restriction(EntityUtils.getIdFieldName(entityClass), id));
        queryBuilder.setAttributeName(attributeName);

        try {
            return queryBuilder.createQuery().getSingleResult();
    ...
    public T unique(Map<String, Object> args) {
        Query query = getQueryFromMap(args);
        try {
    ...
    public T unique(List<Restriction> restrictions, Class clazz) {
        ...
        QueryBuilderOld queryBuilder = createQueryBuilder(clazz);
        queryBuilder.add(restrictions);
    ...
    @Override
    public List<T> list(Map<String, Object> args, String order) {
        Query query = getQueryFromMap(args, order);
        return query.getResultList();
    }
    ...
    public List<T> list(Class clazz, List<Restriction> restrictions, String order, Integer firstResult, Integer maxResults, String attributes) {
        ...
        if (attributes != null && !attributes.isEmpty()) {
            queryBuilder.setAttributeName(attributes);
        }
        queryBuilder.add(restrictions);
        Query query = queryBuilder.createQuery();
        if (maxResults != null) {
    ...
    public Long count(Map<String, Object> args) {
        Query query = getQueryFromMap(args, QueryType.COUNT);
        try {
    ...
    public Long count(List<Restriction> restrictions) {
        QueryBuilderOld queryBuilder = new QueryBuilderOld(entityClass, entityManager, QueryType.COUNT);
        queryBuilder.add(restrictions);
        Query query = queryBuilder.createQuery();
        try {
            return (Long) query.getSingleResult();
    ...
    public Long count(Class clazz) {
        Query query = new QueryBuilderOld(clazz, entityManager, QueryType.COUNT).createQuery();
        return (Long) query.getSingleResult();
    }
    ...
    private QueryBuilderOld createQueryBuilder() {
        return createQueryBuilder(entityClass);
    }

    private QueryBuilderOld createQueryBuilder(Class clazz) {
        return new QueryBuilderOld(clazz, entityManager);
    }
    ...
    private Query getQueryFromMap(Map<String, Object> args) {
        return getQueryFromMap(args, null, null);
    }
    ...
    private Query getQueryFromMap(Map<String, Object> args, String order) {
        return getQueryFromMap(args, null, order);
    }

    private Query getQueryFromMap(Map<String, Object> args, QueryType type) {
        return getQueryFromMap(args, type, null);
    }

    private Query getQueryFromMap(Map<String, Object> args, QueryType type, String order) {
        QueryBuilderOld queryBuilder = createQueryBuilder();
        queryBuilder.setType(type);
        queryBuilder.setOrder(order);
        queryBuilder.add(getRestrictionsFromMap(args));
